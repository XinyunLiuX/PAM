function dydt = active_particles_in_well(t,y,p)
  ...
  dydt(    1:N  ) = ui;
  dydt(  N+1:2*N) = vi;
  dydt(2*N+1:3*N) = Ri.*ui - xi - sum(p.alpha.*deltaxi./(rij.^(p.p+1)),2);
  dydt(3*N+1:4*N) = Ri.*vi - yi - sum(p.alpha.*deltayi./(rij.^(p.p+1)),2);
